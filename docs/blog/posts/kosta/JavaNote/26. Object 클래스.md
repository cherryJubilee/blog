...
> Object 클래스, toString(), equals, hashCode()에 대해서 알아보자
...
- 디버깅과 로깅에 유용하게 사용된다.
- Object가 toString()을 갖고 있으므로 오버라이딩 해서 객체에 대한 정보를 알기 위한 용도로 사용할 수 있다.
...
- Object가 기본으로 제공하는 equals() 는 `==` 으로 동일성 비교를 제공한다. ⭐️
...
-   hashCode()는 인스턴스의 저장 주소를 반환홤 -> 모든 객체가 자신만의 해시 코드를 표현할 수 있는 기능 제공
...
- 보통 오버라이딩(제정의)해서 사용한다. 
- 객체의 참조값을 기반으로 해시코드를 생성한다. 즉 객체의 인스턴스가 다르면 해시코드도 다르다.
